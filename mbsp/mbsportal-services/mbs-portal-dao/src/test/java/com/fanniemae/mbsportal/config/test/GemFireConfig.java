...
public class GemFireConfig {
        ...
        //Dev "10.108.5.207", 10000  10.108.6.180
     /*   private static String locatorHost="10.108.6.50";//"10.108.5.207";//
        private static int locatorPort=10000;
    */
        ...
        public ReflectionBasedAutoSerializer reflectionBasedAutoSerializer() {
                List<String> patterns = Arrays.asList(new String[] { "com.fanniemae.mbsportal.model.*",
                        "com.fanniemae.securitiesods.ods_core.domain.*", "com.fanniemae.mbsportal.id.*","com"
                        + ".fanniemae.fnmpfj.gemfire.utils.client.identity.Sequence#identity=name","com.fanniemae.mbsportal.po.*" });
        ...
        @SuppressWarnings("rawtypes")
        @Bean
        public Region mbspConfigProp() {
                ClientRegionFactory<String, String> cRegionFactory = clientCache()
                        .createClientRegionFactory(ClientRegionShortcut.PROXY);
                Region<String, String> region = cRegionFactory.setPoolName(mtaPoolManager().getName())
                        .create("MBSPConfigProp");
                return region;
        }
        
        @SuppressWarnings("rawtypes")
        @Bean
        public Region mbsPartyRegion() {
                ClientRegionFactory<String, String> cRegionFactory = clientCache()
                        .createClientRegionFactory(ClientRegionShortcut.PROXY);
                Region<String, String> region = cRegionFactory.setPoolName(mtaPoolManager().getName())
                        .create("MBSParty");
        ...
        @SuppressWarnings("rawtypes")
        @Bean
        public Region mbsMarketIndicativePriceHistoryRegion() {
                ClientRegionFactory<String, String> cRegionFactory = clientCache()
                        .createClientRegionFactory(ClientRegionShortcut.PROXY);
                Region<String, String> region = cRegionFactory.setPoolName(mtaPoolManager().getName())
                        .create("MBSMarketIndicativePriceHistory");
                return region;
        }
        ...
        public ClientCache clientCache() {
                ClientCache c = new ClientCacheFactory(gemfireProperties).addPoolLocator(locatorHost,
                        locatorPort) //TODO: make it driven by prop file- 'gemfire-settings.prop' ?
